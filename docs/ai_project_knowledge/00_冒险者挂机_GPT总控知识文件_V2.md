...
## 11. 素材生成与抠图规则（#FF00FF 洋红背景）

### 11.1 核心规则

角色、怪物、宠物、掉落物、动作帧等游戏素材，当前阶段**不要求**生图工具直接输出真透明 PNG。

改为两步工作流：

```text
第一步：生图工具输出纯色背景图（默认 #FF00FF 纯洋红背景）。
第二步：用抠图插件 / 脚本处理成透明 PNG，导入 Godot 4.7。
```
...
| 条件 | 背景色 | 色值 |
|---|---|---|
| 默认 | 纯洋红 Magenta | `#FF00FF` |
| 素材本身含大量粉紫 / 洋红色调 | 备用纯绿色 | `#00FF00` |

背景必须满足：

- 纯色、无渐变、无纹理、无阴影、无光效、无地面
- 单层均匀填充
- 方便抠图插件一键去底
...
UI 组件如能稳定输出真透明可继续用透明背景；不稳定时同样使用 `#FF00FF` 纯色背景方案，扣图后导入。

### 11.5 最终目标（保留）

最终导入 Godot 的正式资源应为透明 PNG / RGBA Alpha。生图阶段不再要求必须直接输出真透明。
